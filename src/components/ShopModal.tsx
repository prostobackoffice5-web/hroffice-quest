  ...
  return (
    <ModalShell title="Лавка" onClose={onClose} wide>
      <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Поиск предметов..." className="pixel-slot px-3 py-1.5 text-sm text-[#f3e9d2] outline-none flex-1 min-w-[160px]" />
      ...
      <div className="flex gap-1 mb-2 flex-wrap">
        <button onClick={() => setCategory('all')} className={`pixel-btn text-xs px-2.5 py-1.5 ${category === 'all' ? 'bg-[#6b4f32] text-white' : 'bg-[#4a3826] text-[#d8c9a8]'}`}>Все</button>
        {(Object.keys(SHOP_CATEGORY_LABELS) as ShopCategory[]).map((c) => (
          <button key={c} onClick={() => setCategory(c)} className={`pixel-btn text-xs px-2.5 py-1.5 ${category === c ? 'bg-[#6b4f32] text-white' : 'bg-[#4a3826] text-[#d8c9a8]'}`}>
            {SHOP_CATEGORY_LABELS[c]}
          </button>
        ))}
      </div>
      <div className="flex gap-1 mb-3 flex-wrap">
        <button onClick={() => setRarity('all')} className={`pixel-btn text-xs px-2.5 py-1 ${rarity === 'all' ? 'bg-[#6b4f32] text-white' : 'bg-[#4a3826] text-[#d8c9a8]'}`}>Любая редкость</button>
        {(Object.keys(RARITY_LABELS) as Rarity[]).map((r) => (
          <button key={r} onClick={() => setRarity(r)} className="pixel-btn text-xs px-2.5 py-1" style={{ background: rarity === r ? RARITY_COLORS[r] : '#4a3826', color: rarity === r ? '#1f150d' : '#d8c9a8' }}>
            {RARITY_LABELS[r]}
          </button>
        ))}
      </div>

      <div className="text-xs text-[#d8c9a8] mb-2">Найдено предметов: {items.length}</div>
        ...
        {items.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[10px] text-[#f3e9d2] text-center leading-tight">{item.name}</span>
              ...
              {owned ? (
                ...
                <>
                  ...
                  <button
                    ...
                    className="pixel-btn bg-[#3f7d3a] disabled:opacity-40 disabled:cursor-not-allowed text-white text-[10px] px-2 py-1"
                  >
                    Купить
                  </button>
                </>
