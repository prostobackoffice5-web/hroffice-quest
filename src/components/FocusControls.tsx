...
export function FocusControls({ task }: { task: Task }) {
    ...
    return (
      <div className="pixel-panel p-2.5 border border-[#facc15]/40">
        <div className="flex items-center justify-between text-xs mb-1">
          ...
          <span className={over ? 'text-[#4ade80]' : 'text-[#f3e9d2]'}>
            {elapsedMin} мин {elapsedSec.toString().padStart(2, '0')} сек {over ? '(время истекло, можно продолжать)' : `/ ${active.plannedMinutes} мин`}
          </span>
        </div>
        <div className="flex gap-1.5">
          <button onClick={() => dispatch({ type: 'PAUSE_FOCUS', player: me })} className="pixel-btn bg-[#4a3826] text-[#f3e9d2] text-xs px-2.5 py-1.5">⏸ Пауза</button>
          <button onClick={() => dispatch({ type: 'STOP_FOCUS', player: me })} className="pixel-btn bg-[#4a3826] text-[#f3e9d2] text-xs px-2.5 py-1.5">Завершить сессию</button>
          <button onClick={() => dispatch({ type: 'COMPLETE_TASK', id: task.id, player: me })} className="pixel-btn bg-[#3f7d3a] text-white text-xs px-2.5 py-1.5">Завершить задачу</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="text-xs text-[#d8c9a8]">
      {totalMinutes > 0 && <div className="mb-1">Затрачено времени: {formatMinutes(totalMinutes)} · сессий: {mySessions.length}</div>}
      {!picking ? (
        <button onClick={() => setPicking(true)} className="pixel-btn bg-[#4a3826] text-[#f3e9d2] text-xs px-2.5 py-1.5">🎯 Начать фокус</button>
      ) : (
        <div className="flex flex-wrap items-center gap-1.5">
          {FOCUS_PRESETS.map((m) => (
            <button key={m} onClick={() => startFocus(m)} className="pixel-btn bg-[#4a3826] text-[#f3e9d2] text-xs px-2 py-1">{m} мин</button>
          ))}
          <input type="number" min={1} value={custom} onChange={(e) => setCustom(Number(e.target.value))} className="w-14 pixel-slot px-1.5 py-1 text-xs text-[#f3e9d2] outline-none" />
          <button onClick={() => startFocus(custom)} className="pixel-btn bg-[#3f7d3a] text-white text-xs px-2 py-1">Старт</button>
          <button onClick={() => setPicking(false)} className="text-[#d8c9a8] px-1">×</button>
        </div>
      )}
        ...
        <div className="pixel-panel p-2 mt-1.5">
          ...
          <div className="flex gap-1.5">
            <button
              ...
              className="pixel-btn bg-[#3f7d3a] text-white text-xs px-2.5 py-1"
            >
              Да, переключиться
            </button>
            <button onClick={() => setConfirmSwitch(null)} className="pixel-btn bg-[#4a3826] text-[#f3e9d2] text-xs px-2.5 py-1">Отмена</button>
          </div>
        </div>
      )}
    </div>
