...
function Block({ top, side, children }: { top: string; side: string; children?: React.ReactNode }) {
  return (
    <div
      className="relative"
      style={{ width: TILE * 0.72, height: TILE * 0.72, background: side, border: '2px solid #1f150d' }}
    >
      <div style={{ position: 'absolute', top: -6, left: 0, right: 0, height: 8, background: top, border: '2px solid #1f150d', borderBottom: 'none' }} />
      {children}
    </div>
  )
}
      ...
      return (
        <div className={common} title={label}>
          <div className="relative">
            ...
            <div style={{ width: 34, height: 30, background: '#2f7d32', border: '2px solid #1f150d', position: 'absolute', top: -26, left: -12 }} />
          </div>
        </div>
      )
    case 'water':
      ...
      return (
        <div className={common} title={label}>
          <Block top="#a97c50" side="#8a5a34">
            <div style={{ position: 'absolute', top: 6, left: 5, width: 10, height: 12, background: '#f3e9d2', border: '1px solid #1f150d' }} />
            <div style={{ position: 'absolute', top: 10, left: 18, width: 10, height: 12, background: '#f3e9d2', border: '1px solid #1f150d' }} />
          </Block>
        </div>
      )
    case 'computer':
      return (
        <div className={common} title={label}>
          <Block top="#8b8f99" side="#5d6270">
            <div style={{ position: 'absolute', top: 4, left: 6, width: 20, height: 14, background: '#1e293b', border: '2px solid #1f150d' }} />
      ...
      return (
        <div className={common} title={label}>
          <Block top="#9c6b2e" side="#6b4a1f">
            <div style={{ position: 'absolute', top: 8, left: '50%', transform: 'translateX(-50%)', width: 8, height: 8, background: '#f5c542', border: '1px solid #1f150d', borderRadius: 2 }} />
          </Block>
        </div>
      )
    case 'shop':
      return (
        <div className={common} title={label}>
          <div className="relative">
            <div style={{ width: 40, height: 10, background: '#b3413c', border: '2px solid #1f150d', clipPath: 'polygon(0 0,100% 0,90% 100%,10% 100%)' }} />
            <div style={{ width: 34, height: 20, background: '#c2a06b', border: '2px solid #1f150d', margin: '0 auto' }} />
          </div>
        </div>
      )
    case 'bed':
      return (
        <div className={common} title={label}>
          <Block top="#e5e7eb" side="#b91c1c">
            <div style={{ position: 'absolute', top: 6, left: 4, width: 14, height: 8, background: '#f8fafc', border: '1px solid #1f150d' }} />
          </Block>
        </div>
      )
    case 'bookshelf':
