      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select value={assignee} onChange={(e) => setAssignee(e.target.value as Assignee)} className="w-full pixel-slot px-2 py-2 outline-none text-[#f3e9d2]">
              <option value="arai" className="bg-[#2b2016]">Арай</option>
              <option value="linara" className="bg-[#2b2016]">Линара</option>
              <option value="both" className="bg-[#2b2016]">Вместе</option>
            </select>
          </div>
          <div />
          <div>
        ...
        <div className="pixel-panel p-3">
          <label className="flex items-center gap-2 text-sm text-[#f3e9d2] font-semibold">
          ...
          {urgent && (
            <div className="grid grid-cols-2 gap-3 mt-2">
              <div className="col-span-2">
                <label className="text-xs text-[#d8c9a8] block mb-1">Срок (дата и время)</label>
                <input type="datetime-local" value={urgentDeadline} onChange={(e) => setUrgentDeadline(e.target.value)} className="w-full pixel-slot px-2 py-2 outline-none text-[#f3e9d2]" />
              </div>
              <div>
                <label className="text-xs text-[#d8c9a8] block mb-1">Бонус опыта</label>
                <input type="number" min={0} value={bonusXp} onChange={(e) => setBonusXp(Number(e.target.value))} className="w-full pixel-slot px-2 py-2 outline-none text-[#f3e9d2]" />
              </div>
              <div>
                <label className="text-xs text-[#d8c9a8] block mb-1">Бонус монет</label>
                <input type="number" min={0} value={bonusCoins} onChange={(e) => setBonusCoins(Number(e.target.value))} className="w-full pixel-slot px-2 py-2 outline-none text-[#f3e9d2]" />
              </div>
              <div>
                <label className="text-xs text-[#d8c9a8] block mb-1">Бонус искр</label>
                <input type="number" min={0} value={bonusSparks} onChange={(e) => setBonusSparks(Number(e.target.value))} className="w-full pixel-slot px-2 py-2 outline-none text-[#f3e9d2]" />
              </div>
            </div>
          )}
        </div>
